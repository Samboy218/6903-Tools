...
def trackFile(fileName, remove=False)
    ...
    if SETTINGS["trackFileList"].get(fileName, None) is not None:
        if remove:
            ...
            send_msg("Stopped tracking" + fileName + "\n")
        else:
            send_msg("File " + fileName + " already tracked\n")
        return
    #otherwise, add the file to our tracked list
    try:
        stats = os.stat(fileName)
    except:
        send_msg("can't stat file, aborting track\n")
        return
    to_track = {
        "last_seen": currTime,
    ...
    send_msg("File "+ fileName +" now being tracked\n")
...
def getTrackedFiles(fName=None):
    ...
    output = ""
    if fName is None:
        for name in SETTINGS["trackFileList"][name].keys()
