...
#the sub bytes in the encrypt file
...
def shift_rows(pt):
	...
	row1s = ''.join(row1)
	row2s = bitWise.shift_right(''.join(row2), 8)
	row3s = bitWise.shift_right(''.join(row3), 16)
	row4s = bitWise.shift_right(''.join(row4), 24)

	return row1s + row2s + row3s + row4s

#Decrypt one standard AES block
#takes a ct, and a round key
#returns a plaintext pt
def decrypt_block(ct, key):
	...
	for x in range(9, -1, -1):
		working_pt = bitWise.xor(round_keys_b[x], working_pt)
		if(x < 9):
			working_pt = mix_cols(working_pt)
		working_pt = shift_rows(working_pt)
		working_pt = sub_bytes(working_pt)
		
	working_pt = bitWise.xor(round_keys_b[0], working_pt)
	pt = working_pt
	return pt
